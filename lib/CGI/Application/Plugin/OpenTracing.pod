...
with multiple values will be joined with the value of
C<$CGI::Application::Plugin::TAG_JOIN_CHAR> package variable (comma by default).
...
of query parameters, form fields or any of them, accordingly.

A special fallback entry can also be specified as an odd element at the end of
the list. That entry will be used as the formatter for tags with no named matches.
This means a fallback specified in C<opentracing_process_tags_query_params>
...
The keys returned by these callbacks are one of:

=over 4

=item * a scalar - match the parameter name exactly

=item * arrayref - match the parameter name exactly to any of the entries

=item * regex - match the parameter to the expression

=back

The following specifications are functionally identical:
...
Example callback:
  ...
  sub opentracing_process_tags {
      ...
      sub { join '-', @_ },                   # join all remaining with a dash
  }
